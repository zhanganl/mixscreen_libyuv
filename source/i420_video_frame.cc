...
int I420VideoFrame::CreateBlackFrame(int width,
	int height,
	int stride_y,
	int stride_u,
	int stride_v)
	...
	memset((void*)const_buffer->data(kYPlane), 0 , width*height);
	memset((void*)const_buffer->data(kUPlane), 128, width*height / 4);
	memset((void*)const_buffer->data(kVPlane), 128, width*height / 4);
	return 0;
}
...
int I420VideoFrame::CopyFrame(const I420VideoFrame& videoFrame) {
  ...
  formate = videoFrame.formate;
  data_len = videoFrame.data_len;
  return 0;
}
...
void I420VideoFrame::SwapFrame(I420VideoFrame* videoFrame) {
  ...
  std::swap(formate, videoFrame->formate);
  std::swap(data_len, videoFrame->data_len);
}
